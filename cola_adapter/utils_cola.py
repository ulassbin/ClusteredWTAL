...
def get_pred_activations(src, pred, config):
    ...
    src_pred = upgrade_resolution(src_pred, config.UP_SCALE) # this is where conversion to time values happens
    return src_pred
...
def upgrade_resolution(arr, scale): # arr is (time, class) size.
    x = np.arange(0, arr.shape[0]) # 0 to time
...
def get_proposal_oic(tList, wtcam, final_score, c_pred, scale, v_len, sampling_frames, num_segments, _lambda=0.25, gamma=0.2):
    ...
    print(f't_factor: {t_factor}, expected t_factor: {16 * v_len / num_segments}')
    temp = []
