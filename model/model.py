...
class ClusterFusion(nn.Module):
    ...
    def forward(self, x):
        ...
        helper.print_memory_usage('After computing cluster distances')
        #print("X {}, distances {}".format(x.device, distances.device))
        x = self.fuse(x, distances)
        helper.print_memory_usage('After fusion')
        return x, distances

class TransformerClusterFusion(nn.Module): # This thing is broken, too much memory usage. forget about this.
    def __init__(self, cfg, cluster_centers):
...
class CrashingVids(nn.Module):
    def __init__(self, cfg, cluster_centers, learnable_cls=False):
    ...
    def forward(self, x):
        ...
        helper.print_memory_usage('Before cluster fusion')
        if(self.transformer_fusion):
            x_fused, _ = self.cluster_fusion_transformer(x)
        else:
            x_fused, _ = self.cluster_fusion_module(x)
        #print('X fused is ', type(x_fused), ' Shape is ', x_fused.shape)
        helper.print_memory_usage('After Cluster Fusion')
        torch.cuda.empty_cache()
        helper.print_memory_usage('After Fusion Empty Cache')
        embeddings, cas, actionness = self.actionness_module(x_fused)
        helper.print_memory_usage('After Actionness Module')
        base_embeddings, base_cas, base_actionness = self.actionness_module(x)
        helper.print_memory_usage('After Base Actionness Module')
        video_scores = self.get_video_cls_scores(cas)
        helper.print_memory_usage('After Video Scores')
        base_vid_scores = self.get_video_cls_scores(base_cas)
        helper.print_memory_usage('After Base Video Scores')
